...
export const level = z.enum(["Без опыта", "Junior"]);
export const format = z.enum(["Удалённо", "Гибрид", "Офлайн"]);
export const specialization = z.enum(["Разработка", "Дизайн", "Продукт", "Аналитика"]);
...
export const profileSchema = z.object({
  name: z.string().trim().min(2).max(80), role, specialization, level, format,
...
export const projectSchema = z.object({
  ...
  specialization, level, format, teamSize: z.number().int().min(1).max(50), published: z.boolean(),
}).strict();
