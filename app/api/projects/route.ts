...
export async function POST(request:Request){
  try{await assertSameOrigin(request);const auth=await requireUser();if(!auth)return noStore({error:"Требуется вход"},{status:401});if(auth.role!=="founder")return noStore({error:"Публиковать проекты могут только основатели"},{status:403});
    ...
    await getDb().execute("INSERT INTO projects (id,owner_id,name,category,title,description,specialization,level,format,team_size,published) VALUES (?,?,?,?,?,?,?,?,?,?,?)",[id,auth.userId,input.name,input.category,input.title,input.description,input.specialization,input.level,input.format,input.teamSize,input.published]);
